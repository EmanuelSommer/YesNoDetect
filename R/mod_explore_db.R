...
mod_explore_db_ui <- function(id) {
  ...
  tagList(
    ...
    tags$br(), tags$br(),
    fluidRow(
      tags$style(".small-box.bg-yellow { background-color: #73777F !important; color: #FFFFFF !important; }"),
    ...
    fluidRow(
      ...
      column(1),
      column(
        width = 4, style = "background-color: #E6EFFF;color: #E6EFFF;",
        plotOutput(ns("label_dist_plot"), width = "400px", height = "300px")
      ),
      column(
        width = 3, style = "background-color:#E6EFFF;color:#E6EFFF;",
    ...
    actionButton(ns("refresh_db"), "Reload database",
...
mod_explore_db_server <- function(input, output, session, r) {
  ...
  output$total_valuebox <- shinydashboard::renderValueBox({
    shinydashboard::valueBox(length(r$current_labels),
      subtitle = "Total boxes", icon = icon("database"),
      color = "yellow", width = NULL
    )
  })
  output$yes_valuebox <- shinydashboard::renderValueBox({
    shinydashboard::valueBox(sum(r$current_labels == "y"),
      subtitle = "Yes boxes", icon = icon("check"),
      color = "green", width = NULL
    )
  })
  output$no_valuebox <- shinydashboard::renderValueBox({
    shinydashboard::valueBox(sum(r$current_labels == "x"),
      subtitle = "No boxes", icon = icon("times"),
