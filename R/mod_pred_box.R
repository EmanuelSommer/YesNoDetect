...
mod_pred_box_ui <- function(id) {
  ...
  tagList(
    ...
    h2(tags$strong("2")),
    h4("Predict"),
    actionButton(ns("predict_button"), "Predict current drawing",
      icon = icon("brain"),
...
mod_pred_box_server <- function(input, output, session, r) {
  ...
  output$yes_result <- shinydashboard::renderValueBox({
    shinydashboard::valueBox(r$pred_yes,
      subtitle = "Probability for yes", icon = icon("check"),
      color = "green", width = 12
    )
  })
  output$no_result <- shinydashboard::renderValueBox({
    shinydashboard::valueBox(r$pred_no,
      subtitle = "Probability for no", icon = icon("times"),
      color = "red", width = 12
    )
  })
}
