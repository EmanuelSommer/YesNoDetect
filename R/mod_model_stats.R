...
mod_model_stats_server <- function(input, output, session){
  ...
  output$size_train <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(value = YesNoDetect::metrics_list$size_training_data,title = "Training samples",icon = icon("layer-group"),
                             color = my_info_color,width = NULL)
  })
  output$size_test <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(value = YesNoDetect::metrics_list$size_test_set,title = "Testing samples",icon = icon("layer-group"),
                             color = my_info_color,width = NULL)
  })
  output$test_acc <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(value = YesNoDetect::metrics_list$test_acc,title = "Accuracy on test set",icon = icon("bullseye"),
                             color = my_info_color,width = NULL)
  })
  output$trainable <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(value = YesNoDetect::metrics_list$trainable_params,title = "Trainable parameters",icon = icon("project-diagram"),
                             color = my_info_color,width = NULL)
  })
  output$loss <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(value = YesNoDetect::metrics_list$loss,title = "Loss function",icon = icon("project-diagram"),
                             color = my_info_color,width = NULL)
  })
  output$optimizer <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(value = YesNoDetect::metrics_list$optimizer,title = "Optimizer",icon = icon("project-diagram"),
                             color = my_info_color,width = NULL)
  })
}
